  ...
  return (
    <div className="w-full py-12 px-4 sm:px-6 lg:px-8" style={{ background: '#0B1221' }}>
      <div className="max-w-4xl mx-auto">
        ...
        {/* Tab Selector */}
        <div className="flex gap-2 mb-0 border-b border-[#1E293B] bg-[#131B2E] rounded-t-xl p-4">
          {tabs.map((tab) => (
                  ...
                  <div key={item.key} onMouseEnter={() => setHighlightedBar(idx)} onMouseLeave={() => setHighlightedBar(null)}
                    ...
                    <div className="w-full h-3 bg-[#1E293B] rounded-full overflow-hidden">
                      <div style={{
                        ...
                        background: item.rate > 50 ? 'linear-gradient(90deg, #0D9488, #14B8A6)' : item.rate > 40 ? 'linear-gradient(90deg, #4F46E5, #6366F1)' : 'linear-gradient(90deg, #E87461, #F59E8C)',
                        transition: 'width 0.6s cubic-bezier(0.16, 1, 0.3, 1)',
                        boxShadow: highlightedBar === idx ? '0 0 12px rgba(99,102,241,0.4)' : 'none',
                      }} />
                    </div>
                  </div>
                ))}
            ...
            <div className="flex items-end gap-2 mb-4" style={{ height: '200px' }}>
              {SETTLEMENT_DATA.map((bar, idx) => (
                <div key={idx} className="flex-1 flex flex-col items-center justify-end h-full group"
                  ...
                  <div className="text-[10px] font-data font-bold mb-1 opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: '#A5B4FC' }}>
                    {bar.count.toLocaleString()}
                  </div>
                  <div className="w-full rounded-t-md transition-all duration-300" style={{
                    height: `${(bar.pct / 28) * 100}%`,
                    background: highlightedBar === idx ? 'linear-gradient(180deg, #6366F1, #4F46E5)' : 'linear-gradient(180deg, rgba(99,102,241,0.6), rgba(79,70,229,0.8))',
                    boxShadow: highlightedBar === idx ? '0 -4px 20px rgba(99,102,241,0.3)' : 'none',
