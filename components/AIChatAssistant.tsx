    ...
    <>
      {!isOpen && (
        <button
          ...
          style={{ backgroundColor: '#0966C3', fontFamily: 'Inter, system-ui, sans-serif' }}
      ...
      {isOpen && (
        <div
          className="fixed bottom-6 right-6 z-50 flex flex-col bg-white border border-gray-200 shadow-2xl overflow-hidden"
        ...
        >
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200" style={{ backgroundColor: '#0966C3' }}>
            ...
            {messages.length === 0 ? (
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {STARTER_QUESTIONS.map((q) => (
                    <button key={q} onClick={() => sendMessage(q)} className="w-full text-left px-4 py-3 rounded-xl text-sm text-gray-700 bg-gray-50 hover:bg-gray-100 transition-colors border border-gray-200">
                      {q}
                    </button>
                  ))}
                </div>
              ...
              messages.map((msg) => (
                <div key={msg.id} className={msg.role === 'user' ? 'flex justify-end' : 'flex justify-start'}>
                  <div
                    className={msg.role === 'user' ? 'max-w-[85%] px-4 py-3 rounded-2xl text-sm text-white' : 'max-w-[85%] px-4 py-3 rounded-2xl text-sm text-gray-800 bg-gray-100'}
                    style={msg.role === 'user' ? { backgroundColor: '#0966C3' } : undefined}
                  >
          ...
          <div className="border-t border-gray-200 px-4 py-3">
            <form onSubmit={onSubmit} className="flex items-center gap-2">
              ...
              <button type="submit" disabled={!inputValue.trim() || isLoading} className="p-2.5 rounded-full text-white transition-colors disabled:opacity-50" style={{ backgroundColor: '#0966C3' }} aria-label="Send message">
