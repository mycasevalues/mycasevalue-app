...
const wrColor = (wr: number) => wr >= 50 ? 'var(--data-positive, #176438)' : wr >= 35 ? '#C37D16' : 'var(--link)';

const CATEGORY_COLORS: Record<string, string> = {
  work: 'var(--text1)', injury: 'var(--link)', consumer: '#70B5F9', rights: 'var(--link)',
  money: '#C37D16', housing: 'var(--data-positive, #176438)', medical: 'var(--data-negative, #B01E1E)', family: '#C37D16',
  gov: 'var(--text2)', education: '#70B5F9', ip: 'var(--data-positive, #176438)', other: 'var(--text2)',
};
  ...
  return (
    <div>
      <style>{`
        ...
        .nos-tr:hover { background: var(--surf) !important; }
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12, marginBottom: 20 }}>
        {[
          { label: 'NOS Codes', value: String(filtered.length), color: 'var(--text1)' },
      ...
      {/* Table */}
      <div className="nos-table-wrap" style={{ background: 'var(--card)', border: '1px solid var(--bdr)', borderRadius: 2, overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'var(--font-ui)', fontSize: 14 }}>
          <thead>
            <tr style={{ background: 'var(--link)' }}>
              <th className="nos-th" onClick={() => toggleSort('nos')} style={{ textAlign: 'left', padding: '12px 16px', color: 'var(--card)', fontWeight: 600, fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.5px', width: 70 }}>
                NOS<SortArrow field="nos" />
              </th>
              <th className="nos-th" onClick={() => toggleSort('label')} style={{ textAlign: 'left', padding: '12px 8px', color: 'var(--card)', fontWeight: 600, fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
              ...
              <th className="nos-th" onClick={() => toggleSort('total')} style={{ textAlign: 'right', padding: '12px 8px', color: 'var(--card)', fontWeight: 600, fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                Cases<SortArrow field="total" />
              </th>
              <th className="nos-th" onClick={() => toggleSort('wr')} style={{ textAlign: 'right', padding: '12px 8px', color: 'var(--card)', fontWeight: 600, fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                Win %<SortArrow field="wr" />
              </th>
              <th className="nos-th" onClick={() => toggleSort('sp')} style={{ textAlign: 'right', padding: '12px 8px', color: 'var(--card)', fontWeight: 600, fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                Settle %<SortArrow field="sp" />
              </th>
              <th className="nos-th" onClick={() => toggleSort('mo')} style={{ textAlign: 'right', padding: '12px 8px', color: 'var(--card)', fontWeight: 600, fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                Duration<SortArrow field="mo" />
              </th>
              <th className="nos-th" onClick={() => toggleSort('rngMd')} style={{ textAlign: 'right', padding: '12px 16px', color: 'var(--card)', fontWeight: 600, fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
            ...
            {filtered.map((row, i) => (
              <>
                <tr
                  ...
                  style={{ borderBottom: '1px solid var(--bdr)', background: i % 2 === 0 ? 'var(--card)' : '#FAFAFA', cursor: 'pointer' }}
                >
                  <td style={{ padding: '8px 16px', fontFamily: 'var(--font-mono)', fontWeight: 600, color: 'var(--accent-primary-hover)', fontSize: 14 }}>
                    {row.nos}
                  </td>
                  <td style={{ padding: '8px 8px', fontWeight: 600, color: 'var(--text1)' }}>
                    {row.label}
                  </td>
                  <td style={{ padding: '8px 8px' }}>
                    <span style={{
                      fontSize: 12, fontWeight: 600, padding: '2px 8px', borderRadius: 2,
                      background: `${CATEGORY_COLORS[row.category] || 'var(--text2)'}15`,
                      color: CATEGORY_COLORS[row.category] || 'var(--text2)',
                      textTransform: 'uppercase',
                    }}>
                      {row.categoryLabel}
                    </span>
                  </td>
                  <td style={{ padding: '8px 8px', textAlign: 'right', fontFamily: 'var(--font-mono)', fontWeight: 600, color: 'var(--text1)' }}>
                  ...
                  <td style={{ padding: '8px 16px', textAlign: 'right', fontFamily: 'var(--font-mono)', fontWeight: 600, color: 'var(--link)' }}>
                ...
                {expandedNos === row.nos && (
                  <tr key={`${row.nos}-expand`}>
                    <td colSpan={8} style={{ padding: 0 }}>
                      <div className="nos-expand" style={{ padding: '16px 24px', background: 'var(--surf)', borderBottom: '2px solid var(--accent-primary-hover)' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 16 }}>
                          <div>
                            <div style={{ fontSize: 12, fontWeight: 600, textTransform: 'uppercase', color: 'var(--text2)', marginBottom: 4 }}>Recovery Range</div>
                            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 14, color: 'var(--text1)' }}>
                              {fmtK(row.rngLo)} – {fmtK(row.rngMd)} – {fmtK(row.rngHi)}
                            </div>
                            <div style={{ fontSize: 12, color: 'var(--text2)', marginTop: 2 }}>25th / Median / 75th</div>
                          </div>
                          <div>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center' }}>
                            <Link
                              ...
                              style={{
                                ...
                                padding: '8px 16px', background: 'var(--link)', color: 'var(--card)',
      ...
      {/* Result count */}
      <div style={{ textAlign: 'center', marginTop: 16, fontSize: 12, color: 'var(--text2)', fontFamily: 'var(--font-ui)' }}>
