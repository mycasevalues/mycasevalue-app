    ...
    <div className="w-full" role="img" aria-label={lang === 'es' ? 'Línea de tiempo del caso' : 'Case timeline'}>
      ...
      <div className="flex flex-wrap gap-x-5 gap-y-2">
        {stages.map((stage, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <div>
              <span className="text-[12px] font-semibold text-slate-600">{stage.label}</span>
      ...
      {totalDuration && (
        <div className="mt-3 pt-3 border-t flex items-center justify-between" style={{ borderColor: '#E2E8F0' }}>
          <span className="text-[12px] font-semibold text-slate-500">{totalLabel || (lang === 'es' ? 'Duración total media' : 'Median total duration')}</span>
          <span className="text-lg font-display font-bold" style={{ color: '#0F172A' }}>{totalDuration}</span>
        </div>
      )}
