    ...
    <div style={{ minHeight: '100vh', backgroundColor: 'var(--surf)' }}>
      ...
      <div style={{ padding: '40px 24px' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '24px' }}>
              {[
                ...
                <div key={item.step} style={{ textAlign: 'center' }}>
                  <div style={{ width: '40px', height: '40px', borderRadius: '50%', backgroundColor: 'var(--link)', color: 'var(--card)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px', fontSize: '20px', fontWeight: 700 }}>
                    {item.step}
                  </div>
                  <h3 style={{ fontSize: '16px', fontWeight: 600, color: 'var(--text1)', margin: '0 0 6px 0' }}>{item.title}</h3>
                  <p style={{ fontSize: '14px', color: 'var(--text2)', margin: 0, lineHeight: 1.5 }}>{item.desc}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ backgroundColor: 'var(--card)', borderRadius: '4px', padding: '32px', boxShadow: 'var(--shadow-xs)', border: '1px solid var(--bdr)', marginBottom: '24px' }}>
            ...
            {loading ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--text2)', fontSize: '14px' }}>
                <div style={{ width: 20, height: 20, border: '2px solid var(--bdr)', borderTopColor: 'var(--link)', borderRadius: '50%', animation: 'spin 0.6s linear infinite' }} />
                Loading...
              ...
              <div>
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                  <code style={{ flex: 1, padding: '12px 16px', backgroundColor: 'var(--surf)', borderRadius: '4px', fontSize: '14px', fontFamily: 'var(--font-mono)', color: 'var(--text1)', border: '1px solid var(--bdr)', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    https://www.mycasevalues.com/?ref={referralCode}
                  </code>
                  <button
                    onClick={handleCopy}
                    style={{ padding: '12px 20px', backgroundColor: copied ? 'var(--data-positive)' : 'var(--link)', color: 'var(--card)', border: 'none', borderRadius: '2px', fontSize: '14px', fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap', transition: 'background-color 200ms' }}
                  >
              ...
              <button
                ...
                style={{ padding: '12px 24px', backgroundColor: 'var(--link)', color: 'var(--card)', border: 'none', borderRadius: '2px', fontSize: '14px', fontWeight: 600, cursor: 'pointer', textTransform: 'uppercase' }}
              >
          ...
          {/* Reward Tiers */}
          <div style={{ backgroundColor: 'var(--card)', borderRadius: '4px', padding: '32px', boxShadow: 'var(--shadow-xs)', border: '1px solid var(--bdr)' }}>
            <h2 className="font-legal" style={{ fontSize: '20px', fontWeight: 600, color: 'var(--text1)', margin: '0 0 20px 0' }}>
              Reward Tiers
            </h2>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
              <thead>
                <tr style={{ borderBottom: '2px solid var(--bdr)' }}>
                  <th style={{ textAlign: 'left', padding: '8px 0', fontWeight: 600, color: 'var(--text2)' }}>Milestone</th>
                  <th style={{ textAlign: 'left', padding: '8px 0', fontWeight: 600, color: 'var(--text2)' }}>Reward</th>
                </tr>
              </thead>
              <tbody>
                {rewards.map((r) => (
                  <tr key={r.count} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    <td style={{ padding: '12px 0', fontWeight: 500, color: 'var(--text1)' }}>{r.count}</td>
