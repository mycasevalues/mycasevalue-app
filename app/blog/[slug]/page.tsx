    ...
    <div className="min-h-screen" style={{ background: 'var(--bg-base)' }}>
      ...
      <div className="border-b" style={{ borderColor: 'var(--border-default)', background: 'linear-gradient(180deg, #FFFFFF 0%, var(--bg-base) 100%)' }}>
        <div className="max-w-3xl mx-auto px-6 py-12">
          <a href="/blog" className="inline-flex items-center gap-2 text-sm font-semibold mb-6 transition-colors hover:opacity-80" style={{ color: '#212529' }}>
          ...
          {/* Title */}
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-display font-extrabold mb-6" style={{ color: '#212529', letterSpacing: '-1.5px' }}>
            {post.title}
          </h1>

          {/* Description */}
          <p className="text-lg leading-relaxed" style={{ color: '#455A64' }}>
            {post.description}
          </p>

          {/* Meta information */}
          <div className="flex flex-wrap items-center gap-4 mt-8 pt-6 border-t" style={{ borderColor: 'var(--border-default)' }}>
            <div className="text-sm" style={{ color: '#455A64' }}>
              <span className="font-medium" style={{ color: '#212529' }}>{post.author}</span>
      ...
      <article className="max-w-3xl mx-auto px-6 py-16">
        <div className="prose max-w-none" style={{
          ...
          {post.content.split('\n\n').map((paragraph, idx) => (
            <p key={idx} className="text-lg leading-relaxed mb-6" style={{ color: '#455A64' }}>
              {paragraph}
            </p>
          ))}
        </div>
            ...
            {relatedPosts.map((relatedPost) => (
              ...
              >
                <div className="flex flex-col gap-3 h-full">
                  ...
                  <h3 className="text-base font-display font-bold leading-tight group-hover:text-blue-400 transition-colors" style={{ color: '#212529' }}>
                    {relatedPost.title}
                  </h3>
                  <p className="text-sm leading-relaxed flex-1" style={{ color: '#455A64' }}>
                    {relatedPost.description}
                  </p>
                  <div className="text-xs pt-2 border-t" style={{ color: '#455A64', borderColor: 'var(--border-default)' }}>
