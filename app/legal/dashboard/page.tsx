...
const EVENT_STYLES: Record<string, { bg: string; color: string; icon: string }> = {
  ingestion: { bg: '#E8F4FD', color: 'var(--link)', icon: '\u2B07' },
  processing: { bg: '#F5F3FF', color: 'var(--link)', icon: '\u2699' },
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '24px 16px 40px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, marginBottom: 28 }}>
        <div style={{ padding: '24px 24px', borderRadius: 4, border: '1px solid var(--bdr)', background: 'var(--card)' }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 4 }}>Total Documents</div>
          <div style={{ fontSize: 28, fontWeight: 700, color: 'var(--link)', fontFamily: 'var(--font-mono, monospace)' }}>{formatNumber(totalDocs)}</div>
          <div style={{ fontSize: 12, color: 'var(--data-positive)', marginTop: 4 }}>+12,341 today</div>
        </div>
        <div style={{ padding: '24px 24px', borderRadius: 4, border: '1px solid var(--bdr)', background: 'var(--card)' }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 4 }}>Sources Online</div>
        ...
        <div style={{ padding: '24px 24px', borderRadius: 4, border: '1px solid var(--bdr)', background: 'var(--card)' }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 4 }}>Avg Uptime</div>
          <div style={{ fontSize: 28, fontWeight: 700, color: 'var(--text1)', fontFamily: 'var(--font-mono, monospace)' }}>{avgUptime}%</div>
          <div style={{ fontSize: 12, color: 'var(--color-text-muted)', marginTop: 4 }}>30-day rolling</div>
        </div>
        <div style={{ padding: '24px 24px', borderRadius: 4, border: '1px solid var(--bdr)', background: 'var(--card)' }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 4 }}>Avg Latency</div>
          <div style={{ fontSize: 28, fontWeight: 700, color: 'var(--text1)', fontFamily: 'var(--font-mono, monospace)' }}>{avgLatency}ms</div>
          <div style={{ fontSize: 12, color: 'var(--color-text-muted)', marginTop: 4 }}>API response time</div>
        </div>
      </div>

      {/* Tab nav */}
      <div style={{ display: 'flex', gap: 4, marginBottom: 24, borderBottom: '1px solid var(--bdr)', paddingBottom: 0 }}>
        {(['overview', 'events', 'quality'] as const).map(tab => (
          <button
            ...
            style={{
              ...
              color: activeTab === tab ? 'var(--link)' : 'var(--color-text-muted)',
              background: 'transparent',
              border: 'none',
              borderBottom: activeTab === tab ? '2px solid var(--link)' : '2px solid transparent',
              cursor: 'pointer',
            ...
            return (
              ...
              >
                ...
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--text1)', fontFamily: 'var(--font-mono, monospace)' }}>
                    {formatNumber(src.documents)}
                  </div>
                  <div style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>docs</div>
                </div>
                ...
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 600, color: src.uptime >= 99 ? 'var(--data-positive)' : src.uptime >= 97 ? 'var(--wrn-txt)' : '#B91C1C' }}>
                      {src.uptime}%
                    </span>
                  </div>
                ...
                {/* Latency */}
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: src.latency === 0 ? 'var(--data-negative)' : src.latency < 300 ? 'var(--text1)' : 'var(--wrn-txt)', fontFamily: 'var(--font-mono, monospace)' }}>
                    {src.latency === 0 ? '---' : `${src.latency}ms`}
                  </div>
                  <div style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>latency</div>
                </div>

                {/* Last sync */}
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>{src.lastSync}</div>
                  <div style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>every {src.syncInterval}</div>
                </div>
              </div>
            );
          })}
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 2 }}>
                    <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text1)' }}>{evt.source}</span>
                    <span style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>{evt.timestamp}</span>
                  </div>
                  <div style={{ fontSize: 14, color: 'var(--text2)', lineHeight: 1.5 }}>{evt.message}</div>
                  {evt.count && (
                    <span style={{
                      display: 'inline-block',
                      marginTop: 6,
      ...
      {activeTab === 'quality' && (
        <div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 16, marginBottom: 24 }}>
            {/* Completeness */}
            <div style={{ padding: '24px', borderRadius: 4, border: '1px solid var(--bdr)', background: 'var(--card)' }}>
              <h3 style={{ fontSize: 16, fontWeight: 600, color: 'var(--text1)', margin: '0 0 16px' }}>Document Completeness</h3>
              {[
                ...
                <div key={item.label} style={{ marginBottom: 12 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    ...
                    <span style={{ fontSize: 14, fontWeight: 600, color: item.value >= 95 ? 'var(--data-positive)' : item.value >= 90 ? 'var(--wrn-txt)' : 'var(--data-negative)' }}>{item.value}%</span>
                  </div>
                  <div style={{ height: 6, borderRadius: 4, background: 'var(--bdr)' }}>
                    <div style={{
                      height: '100%',
                      width: `${item.value}%`,
                      borderRadius: 4,
            ...
            {/* Freshness */}
            <div style={{ padding: '24px', borderRadius: 4, border: '1px solid var(--bdr)', background: 'var(--card)' }}>
              <h3 style={{ fontSize: 16, fontWeight: 600, color: 'var(--text1)', margin: '0 0 16px' }}>Data Freshness</h3>
              {sources.map(src => {
                const fresh = src.status === 'healthy' ? 'current' : src.status === 'degraded' ? 'stale' : 'unavailable';
                const freshColor = fresh === 'current' ? 'var(--data-positive)' : fresh === 'stale' ? 'var(--wrn-txt)' : 'var(--data-negative)';
                return (
                  <div key={src.key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    <span style={{ fontSize: 14, color: src.color, fontWeight: 500 }}>{src.name}</span>
          ...
          {/* Processing stats */}
          <div style={{ padding: '24px', borderRadius: 4, border: '1px solid var(--bdr)', background: 'var(--card)' }}>
            <h3 style={{ fontSize: 16, fontWeight: 600, color: 'var(--text1)', margin: '0 0 16px' }}>Processing Pipeline</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
              {[
                { stage: 'Ingestion Queue', count: 3421, status: 'active', color: 'var(--link)' },
                { stage: 'Text Extraction', count: 892, status: 'active', color: 'var(--link)' },
                { stage: 'Entity Recognition', count: 456, status: 'active', color: 'var(--data-positive)' },
                ...
                <div key={stage.stage} style={{ textAlign: 'center', padding: '16px' }}>
                  ...
                  <div style={{ fontSize: 14, color: 'var(--text2)', marginBottom: 4 }}>{stage.stage}</div>
                  <span style={{
                    display: 'inline-block',
                    padding: '2px 8px',
                    borderRadius: 4,
