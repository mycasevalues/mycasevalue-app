...
export function getWinRateColor(rate: number): {
  ...
  if (rate >= 50) return { bg: '#EDF3FB', text: 'var(--link, #0A50A2)', border: 'var(--link, #0A50A2)', label: 'Favorable' };
...
export function getSettlementColor(medianInThousands: number): string {
  if (medianInThousands >= 500) return 'var(--link, #0A50A2)';
  if (medianInThousands >= 100) return 'var(--link, #0A50A2)';
