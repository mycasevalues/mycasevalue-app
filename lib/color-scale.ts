...
export function getWinRateColor(rate: number): {
  ...
  if (rate >= 50) return { bg: '#EDF3FB', text: 'var(--link, #1A73E8)', border: 'var(--link, #1A73E8)', label: 'Favorable' };
...
export function getSettlementColor(medianInThousands: number): string {
  if (medianInThousands >= 500) return 'var(--link, #1A73E8)';
  if (medianInThousands >= 100) return 'var(--link, #1A73E8)';
