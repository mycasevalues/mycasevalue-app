...
function generateFJCDataFromRealData(): {
  ...
  for (const [nosCode, caseData] of Object.entries(REAL_DATA)) {
    const stat = {
      ...
      avgMonetaryAward: 0,
      totalMonetaryAward: 0,
      median_settlement: caseData.rng?.md ? caseData.rng.md * 1000 : 0,
      settlement_lo: caseData.rng?.lo ? caseData.rng.lo * 1000 : 0,
      settlement_md: caseData.rng?.md ? caseData.rng.md * 1000 : 0,
      settlement_hi: caseData.rng?.hi ? caseData.rng.hi * 1000 : 0,
      represented_win_rate: caseData.rr?.wr || 0,
      represented_total: caseData.rr?.total || 0,
      pro_se_win_rate: caseData.ps?.wr || 0,
